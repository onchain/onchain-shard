...
class Ethereum
  ...
  def self.create(to : String, from : String, amount : UInt64, gas_price : UInt64, gas_limit : UInt64)
    ...
    response = HTTP::Client.get "https://onchain.io/api/ethereum/create//?to=#{to}&from=#{from}&amount=#{amount}&gas_price=#{gas_price}&gas_limit=#{gas_limit}"
  ...
  def self.sign_and_send(to : String, from : String, amount : UInt64, r : String, s : String, v : String, gas_price : UInt64, gas_limit : UInt64)
    ...
    response = HTTP::Client.get "https://onchain.io/api/ethereum/sign_and_send//?to=#{to}&from=#{from}&amount=#{amount}&r=#{r}&s=#{s}&v=#{v}&gas_price=#{gas_price}&gas_limit=#{gas_limit}"
